...
CHDeclareMethod0(void, MMUIViewController, messageCallBack){
    ...
    if(self == [[LKNewestMsgManager sharedInstance]getCurrentVC] && ![currentChatName isEqual: [LKNewestMsgManager sharedInstance].username]){
        ...
        NSAttributedString *content = [[NSAttributedString alloc] initWithString:[LKNewestMsgManager sharedInstance].content attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:12]}];
        CGRect rect = [content boundingRectWithSize:CGSizeMake(self.view.frame.size.width-20, 40) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading context:nil];
        rect.size.width +=  5;
...
CHOptimizedMethod2(self, void, CMessageMgr, AsyncOnAddMsg, NSMutableString*, msg, MsgWrap, CMessageWrap*, wrap){
    if(![wrap.m_nsPushContent isEqual: @""] && wrap.m_nsPushContent != NULL){
        ...
        NSString *name =  [[HelperConfig shareConfig] getDisplayName:wrap];
        NSString *content = [name stringByAppendingFormat:@":%@", wrap.m_nsPushBody];
        [LKNewestMsgManager sharedInstance].content = (id)content;
